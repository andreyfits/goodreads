...
$(document).ready(function () {
    ...
    $(".captcha img:first-child").bind("click", function (event) {
        var captcha = $(".captcha img:last-child"); //явл. посл. дочерним элементом
        var src = $(captcha).attr("src");
        if ((i = src.indexOf("?")) == -1) src += "?" + Math.random(); //если у нас еще знака ? нет, то доб. знак ? и случайное число
        else src = src.substring(0, i) + "?" + Math.random(); //иначе заменяем
        $(captcha).attr("src", src); //и обновляем атрибут src
    });

...
{ //класс для вывода рез-та поиска
    ...
    {
        $query = $this->query;
        $query = mb_strtolower($query); //приводим к ниж. регистру
        $query = preg_replace("/ {2,}/", " ", $query); //все повт. пробелы на один пробел
    ...
    private function getDescription($text, $query)
    {
        ...
        if (strlen($text) > $len) {
            $i = 0;
            $k = 0;
            $array_words = explode(" ", $query); //получаем массив слов
            $pos = array();
            foreach ($array_words as $key => $value) {
                while (strpos($text, $value, $i) !== false) {
                    $pos[$k] = strpos($text, $value, $i);
                    $i += $pos[$k] + 1; //увел. i, для находжения на нов. месте
            ...
            if (count($pos) != 0) {
                if (count($pos) > 1) {
                    ...
                    for ($i = 0; $i < count($pos); $i++) {
                        ...
                        for ($j = $i; $j < count($pos) - 1; $j++) {
                            $sum += $pos[$j + 1] - $pos[$j]; //ищем сумму вхождений дистанций
                            if ($sum <= $len) $k++;
                            else break;
                            $temp_freq[$k] = $pos[$j + 1];
                        }
                ...
                $free_space = $len - ($max_freq[$max] - $max_freq[0]); //расстояние между первым и посл. эл.
    ...
    { //метод полученного рез-та поиска
